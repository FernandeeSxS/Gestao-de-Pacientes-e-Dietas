...
#pragma region TÓPICO 1. b.
  ...
  *
  * Esta função recebe um nome de um ficheiro e um array de structs Dieta,
  * e exporta os dados das dietas para o ficheiro.
  *
  * @param fileName é o nome do ficheiro que vai ser feita a exportação.
...
/*
#pragma region TÓPICO 1. b. (extra)
/**
 * @brief Lê os dados das dietas de um ficheiro CSV ou TXT.
 *
 * Esta função recebe um nome de ficheiro e lê os dados das dietas do ficheiro no formato CSV ou TXT.
 * Os dados lidos são armazenados num array de structs Dieta.
 *
 * @param fileName é o nome do ficheiro para leitura.
 * @param dietas é o array de structs Dieta para armazenar os dados lidos.
 * @param maximodietas é o número máximo de dietas no array.
...
bool LeDadosDieta(char fileName[], Dieta dietas[], int maximodietas) {
	...
	while (i < maximodietas) {
		...
		i++;
		if (feof(fp)) break; //Condição de paragem
	}
	fclose(fp);
	return true;
}
 ...
 *
 * Esta função recebe um nome de ficheiro e exporta os dados do plano nutricional
 * para o ficheiro no formato CSV ou TXT.
 *
 * @param fileName é o nome do ficheiro para exportação.
 ...
 */
 ...
 //Esta função tem a mesma estrutura que a realizada no ponto 1- b)
...
/**
 * @brief Lê os dados dos planos nutricionais de um ficheiro e armazena-os num array de estruturas PlanoNutri.
 *
 * Esta função recebe um nome de ficheiro e lê os dados das dietas do ficheiro no formato CSV ou TXT.
 * Os dados lidos são armazenados num array de structs PlanoNutri.
 *
 * @param fileName é o nome do ficheiro a ser lido.
...
int NumPacientesUltrapassamCal(PlanoNutri plano[], Dieta dietas[], int maximopacientes) {
	...
	for (int i = 0; i < maximopacientes; i++) {  // Itera sobre o array de pacientes
		if (dietas[i].calorias > plano[i].caloriasMax) {
			contador++; // Se o consumo de calorias na dieta atual excede o limite definido no plano é somado 1 unidade à variável contador 
...
void OrdenaPacientesForaIntervalo(NaoCumpPaciente dadospacientes[], int maxpaciente) {
	for (int i = 0; i < maxpaciente - 1; i++) {
		for (int j = i + 1; j < maxpaciente; j++) {
			if (dadospacientes[i].numpaciente < dadospacientes[j].numpaciente) {  // Compara os números dos pacientes para determinar a ordem
...
/**
 * @brief Lista o plano nutricional para um paciente numa refeição específica durante um determinado período.
 *
 * Esta função imprime na tela o plano nutricional para um paciente numa refeição específica durante um período definido.
 *
...
void ListaPlanoNutricional(PlanoNutri planos[], int maxplanos, int numpaciente, int refeicao, char dataInicio[], char dataFim[]) {
	...
	//Criação de um variável e colocação da "string" correspondente a cada indice para ser imprimido na consola o valor em "string"
...
void CalcularMediasRefeicoes(Dieta dietas[], MediaPaciente dados[], int maxdietas, int maxpacientes, char dataInicio[], char dataFim[]) {
	...
	for (int i = 0; i < maxpacientes; i++) {
		...
		//Se a variável auxiliar num (PequenoAlmoco,Almoco,Jantar) for maior que zero, armazena os dados na struct MediaPaciente. Caso contrário retorna 0
		dados[i].numpaciente = i + 1; //Pacientes são numerados a partir de 1
